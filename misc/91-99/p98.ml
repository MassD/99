...
*)
let ints i n = let rec gen j acc = if j = n then acc else gen (j+1) (i::acc) in gen 0 []
...
let solve l1 l2 =
  ...
  let rec run rows_acc seqs seq = 
    match seq with
      | [] -> []
      | seq_hd::seq_tl ->
	let rows = rows_acc@[seq_hd] in
	match seqs with
	  | [] -> if valid (=) l2 rows then rows else run rows_acc [] seq_tl
	  | seqs_hd::seqs_tl ->
	    if valid (<=) l2 rows then 
	      let r = run rows seqs_tl seqs_hd in
	      if r = [] then run rows_acc seqs seq_tl
	      else r
	    else run rows_acc seqs seq_tl
  in 
